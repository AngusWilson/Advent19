...
.d3.p1:{
    ...
    wirePaths:genPositions each wires;

    min (sum each (inter) . wirePaths) except 0
    }
...
.d3.p2:{
    ...
    wirePaths:genPositions each wires;
    intersections:(inter) . wirePaths;

    first min sum {(where each x ~\:/: 1_y)}[;intersections] each wirePaths
    }
